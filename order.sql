...
SELECT customer_id,count(customer_id),sum(total_amount) FROM orders
    GROUP BY customer_id HAVING count(customer_id) > 2;

--  7: Find the total amount of orders placed each month in the year 2022.
SELECT extract(month FROM order_date)as month,sum(total_amount)as totalAmount FROM orders 
